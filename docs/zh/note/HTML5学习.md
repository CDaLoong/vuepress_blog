...
3. contenteditable  将文本改为输入框，在页面上可以修改，可继承，<!--没有兼容性问题，都可以使用-->
   默认值false，不可修改，true会将文字改为一个文本框，可在页面上进行修改，可以将整个标签都删除，尽量不要嵌套使用

4. draggable 使属性盒子可以拖拽 <!--chrome、Safari支持，Firefox极少版本浏览器支持-->
   ...
   <div draggable="true"></div> <!--被拖拽物体-->
   <div class="target"></div><!--目标区域-->
   ```js```
   <!-- 拖拽开始触发的对应事件 -->
