...
- github地址：[https://github.com/DuYi-Edu/DuYi-VUE](https://github.com/DuYi-Edu/DuYi-VUE)
- 下载并安装vue-devtools插件
- 由于某些原因，下载不了谷歌商店上的插件，所以做好科学上网
- 安装插件：[https://github.com/qin-ziqi/Chrome-SetupVPN-3.7.0](https://github.com/qin-ziqi/Chrome-SetupVPN-3.7.0) ，跟着上面的步骤做，插件安装较慢，据不完全统计，约需30分钟
- 安装好了后，点击谷歌浏览器的更多工具 -> 扩展程序 -> 打开chrome网上应用商店 -> 搜索vue devtools -> 添加至chrome
- 是否生效？访问：[https://www.bilibili.com/](https://www.bilibili.com/) ，图标变绿了就生效了
...
```
   ...
   -  绑定style 
      - 使用对象语法
        ...
        但是使用短横线分隔时，要用引号括起来  也可以直接绑定一个样式对象，这样模板会更清晰：  
...
url: [https://sp0.baidu.com/5a1Fazu8AA54nxGko9WTAnF6hhy/su](https://sp0.baidu.com/5a1Fazu8AA54nxGko9WTAnF6hhy/su)
...
很不幸，Vue官方已不再维护这个库了，so...哈哈哈，我们再学点其他的୧[ _ ಡ ▽ ಡ _ ]୨
...
![](https://developer.duyiedu.com/myVue/template.png#id=noc2L&originHeight=1744&originWidth=1200&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)
...
<!-- ![](https://developer.duyiedu.com/myVue/lifecycle1.png#id=fnfqH&originHeight=2850&originWidth=1200&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=) -->

## 生命周期钩子

所有的生命周期钩子自动绑定 this 上下文到实例中，因此你可以访问数据，对属性和方法进行运算

<!-- ![](https://developer.duyiedu.com/myVue/lifecycle2.png#id=XR6Iw&originHeight=2881&originWidth=1200&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=) -->
...
baseURL: '[https://developer.duyiedu.com/vue/bz](https://developer.duyiedu.com/vue/bz)'
url: '/banner'
...
![](https://developer.duyiedu.com/myVue/data.gif#id=Lz7NX&originHeight=70&originWidth=696&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)
...
并且 v-on 事件监听器在 DOM 模板中会被自动转换为全小写，所以 [@myEvent ](/myEvent ) 将会变成 @myevent，导致 myEvent 不可能被监听到。 
...
可以看到，此时组件的根元素实际上是一个元素，那么父级的.native监听器将静默失败。它不会产生任何报错，但是`onFocus`处理函数不会如预期被调用。
...
<!-- ![](https://cn.vuejs.org/images/dom-tree.png#id=QG4QB&originHeight=860&originWidth=1352&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=) -->
...
图示：
<!-- ![](https://cn.vuejs.org/images/transition.png#id=yEYAj&originHeight=600&originWidth=1200&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=) -->
...
Animate.css 官网地址：[https://daneden.github.io/animate.css/](https://daneden.github.io/animate.css/)
...
Velocity.js 官网地址：[http://velocityjs.org/](http://velocityjs.org/)
...
- 不同于 ，它会以一个真实元素呈现：默认为一个 `<span>`。你也可以通过 tag attribute 更换为其他元素。
...
<!-- ![](https://vuex.vuejs.org/vuex.png#id=Bpc01&originHeight=551&originWidth=701&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=) -->
